...
def summarize_single_thread(conversation_id: str, output_dir: str = "./summaries") -> Dict:
    ...
    Args:
        conversation_id (str): The ID of the conversation to summarize
        output_dir (str): Directory to save JSON summary
        
    Returns:
        Dict: Summary results including status and content
    """
    try:
...
def load_thread_summary(conversation_id: str, output_dir: str = "./summaries") -> Optional[Dict]:
    ...
    Args:
        ...
        output_dir (str): Directory containing JSON summaries
        
    Returns:
        Optional[Dict]: The summary if found, None otherwise
    """
    try:
        filename = generate_filename(conversation_id)
...
def get_or_create_thread_summary(conversation_id: str, output_dir: str = "./summaries", force_refresh: bool = False) -> Dict:
    ...
    Args:
        conversation_id (str): The ID of the conversation to summarize
        output_dir (str): Directory for JSON summaries
        force_refresh (bool): If True, regenerate summary even if it exists
        
    Returns:
        Dict: Thread summary with status and content
    """
    try:
...
if __name__ == "__main__":
    ...
    conversation_id = "AAQkADllMTVhOTNmLTFjMDYtNDllMS1iM2Y3LTEzZjlkZjQxMWE5NgAQANnwW-2YrkRJi3vZ7aNvfU8="  # This would come from Streamlit
    ...
    try:
        ...
        if summary['status'] == 'success':
            logger.info(
                f"Thread Summary - Conversation ID: {summary.get('conversation_id')}, "
